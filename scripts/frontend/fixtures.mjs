...
export const models = modelNames.map((name, index) => ({
    ...
    condition: { ID: index + 201, name: 'default', parameters: {} },
...
export function workspace(state, body) {
    ...
    const options = state.comparisonModels ? comparisonOptions(state.comparisonModels, body.comparison) : null;
    ...
    return {
        ...
        publicPie: { participantCount: 128, isFallback: false, fallbackRules: state.empty ? [] : state.publicFallbackRules,
        ...
        comparison: options ?? { mode: body.comparison?.mode ?? 'best', keys: body.comparison?.keys ?? [], availableKeys: ['model'], unconfiguredCount: 0 },
        modelLeaderboards: options ? { public: scoreComparison(state.comparisonModels, publicWeights, 10000, {}, options), personal: scoreComparison(state.comparisonModels, state.personalEntries, 10000, {}, options) } : { public: state.empty ? [] : models, personal: state.empty || !state.authenticated ? [] : models.map(item => ({ ...item, lower: item.lower - 2 })) },
...
export function catalog() {
    return {
        ...
        models: models.map(item => ({ ...item, ID: item.modelID, conditions: [{ ID: item.ID, name: 'default', key: 'default', isDefault: true, parameters: {} }] })),
...
export function approvedResults(body) {
    ...
    return { model: { modelID: model.modelID, modelConditionID: model.ID, modelName: model.name, vendorName: model.vendorName, modelConditionName: 'default' },
        scoreGroups: benchmarks.slice(0, 5).map((item, index) => ({
            ...
            medianRawScore: 89 - index * 2, medianNormalizedScore: 89 - index * 2, usesPercentageScale: true, sampleCount: 2, distinctScoreCount: 2,
            samples: [0, 1].map(sample => ({ ID: 501 + index * 2 + sample, rawScore: 88 - index * 2 + sample * 2, normalizedScore: 88 - index * 2 + sample * 2,
                sourceURL: 'https://example.com/evidence', excludedAsDuplicate: false, sourceTitle: 'Evaluation report (synthetic fixture)', sourceType: 'independent', createdAt: '2026-09-01T08:00:00Z' }))
        })) };
}
